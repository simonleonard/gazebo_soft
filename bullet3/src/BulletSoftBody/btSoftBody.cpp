...
{
  //std::cout<<"seeeeeeeeeeeeeting  velocity: "<<velocity.getX()<<" "<<velocity.getY()<<" "<<velocity.getZ()<<" "<< std::endl;
 
    
  // std::cout<<"set velocity 1:" <<m_nodes[0].m_x.getX()<<std::endl;
	for(int i=0,ni=m_nodes.size();i<ni;++i) 
	{
		Node&	n=m_nodes[i];
		//std::cout<<m_nodes[i].m_im;
		if(n.m_im>0)
		{
			n.m_v	=	velocity;
			}
	...
	//std::cout<<"\nset Velocity  :" <<m_nodes[0].m_v.getX()<<"masssss: "<<std::endl;
...
{ 
  //std::cout<<"adding  velocity before : "<<velocity.getX()<<" "<<velocity.getY()<<" "<<velocity.getZ()<<" "<< std::endl;
	Node&	n=m_nodes[node];
	if(n.m_im>0)
	{
		...
		//std::cout<<"adding  velocity after : "<<velocity.getX()<<" "<<velocity.getY()<<" "<<velocity.getZ()<<" "<< std::endl;
}
...
{
  //std::cout<<"predict motion  :" <<m_nodes[0].m_v.getX()<<std::endl;
	int i,ni;
	...
	if(m_bUpdateRtCst)
	{
        ...
        //std::cout<<"predict motion 1 :" <<m_nodes[0].m_x.getX()<<std::endl;
	...
	//sketchy, hard coding in the gravity. 
	m_worldInfo->m_gravity = btVector3(0,0,-10);

        //std::cout<<"predict motion 2 :" <<m_nodes[0].m_x.getX()<<std::endl;

	//std::cout<<"gravity: "<<m_worldInfo->m_gravity.getX()<<" "<<m_worldInfo->m_gravity.getY()<<" "<<m_worldInfo->m_gravity.getZ()<<" "<<std::endl;
	...
	//	std::cout<<"predict motion 3 :" <<m_nodes[0].m_x.getX()<<std::endl;
	...
	//std::cout<<"predict motion 4 :" <<m_nodes[0].m_x.getX()<<std::endl;
	
	/* Clusters				*/ 
	updateClusters();
        
	//std::cout<<"predict motion 5 :" <<m_nodes[0].m_x.getX()<<std::endl;
	
	/* Bounds				*/ 
	updateBounds();	
        
	//std::cout<<"predict motion 6 :" <<m_nodes[0].m_x.getX()<<std::endl;
	...
	//std::cout<<"predict motion 7 :" <<m_nodes[0].m_x.getX()<<std::endl;
	...
	//std::cout<<"predict motion 8 :" <<m_nodes[0].m_x.getX()<<std::endl;
	
	/* Pose					*/ 
	updatePose();
        
	//std::cout<<"predict motion 9 :" <<m_nodes[0].m_x.getX()<<std::endl;
	...
	//std::cout<<"predict motion 10 :" <<m_nodes[0].m_x.getX()<<std::endl;
	
	/* Clear contacts		*/ 
	m_rcontacts.resize(0);
	m_scontacts.resize(0);
        
	//std::cout<<"predict motion 11 :" <<m_nodes[0].m_x.getX()<<std::endl;
...
{
	/* Update			*/ 
  //std::cout<<this->m_tag<<": tag of  softbody being integrated\n";
	updateNormals();
}
...
{
  //std::cout<<"btSoftBody::RayFromToCaster::Process()\n";
...
/*
  In this attempt I tried to use the contact manifold to access the location of the contact point, and then cut the nearest link.
  The first part (finding the contact location using the manifold) worked, but I could not select out the contact point based on 
  the type of the body (contactManifold->getBody*() returns a btRigidBody even for the softbody), so I couldn't determine which 
  contact location belonged to the softbody. However, the link-distance/cutting part of the method might be functional given a 
  correct location. 
void btSoftBody::cutOnContact() 
{

        int i;
        int numManifolds = m_worldInfo->m_dispatcher->getNumManifolds();
	std::cout<< "m_worldInfo dispatcher: "<<typeid(m_worldInfo->m_dispatcher).name()<<std::endl;
        //iterates through all the manifolds
        for (i=0;i<numManifolds;i++)
        {

                btPersistentManifold* contactManifold = m_worldInfo->m_dispatcher->getManifoldByIndexInternal(i);
                const btCollisionObject* obA = contactManifold->getBody0();//static_cast<btCollisionObject*>(contactManifold->getBody0());
                const btCollisionObject* obB = contactManifold->getBody1();//static_cast<btCollisionObject*>(contactManifold->getBody1());

                int numContacts = contactManifold->getNumContacts();
                //iterates through all the contacts in each manifold
                for (int j=0;j<numContacts;j++){
                        ...
                        //std::cout<<"ptA: [" << ptA.getX()<<", "<< ptA.getY()<<", "<< ptA.getZ()<<"}     "<<"ptB: [" << ptB.getX()<<", "<< ptB.getY()<<", "<< ptB.getZ()<<"}\n"                        <<std::endl;
			//std::cout<< "obA: "<<typeid(*obA).name()<<"\nobB: "<<typeid(*obA).name()<<std::endl;

                        btVector3 nodePose;
                        double minDist;
                        int linkToCutIndex;//link to cut
			std::string obA_name = typeid(*obA).name();
			std::string obB_name = typeid(*obB).name();
                        // Check if obA or obB are softbodies and if so find the closest link to the contact point and then cut that link and add it to cutLinks array and cut the link.
                        if (obA_name.compare("10btSoftBody")==0 ||
			    obB_name.compare("10btSoftBody")==0) {
			  //std::cout<< "obA: "<<typeid(*obA).name()<<"\nobB: "<<typeid(*obA).name()<<std::endl;

                              
			  //iterate through links to find closest one to contact point
                                for(int i = 0; i<m_links.size();i++){

                                        btVector3 node1Pose = m_links[i].m_n[0]->m_x;
                                        double node1Dist = node1Pose.distance(ptA);
                                        btVector3 node2Pose = m_links[i].m_n[1]->m_x;
                                        double node2Dist = node2Pose.distance(ptA);
                                        double linkDist = node1Dist+node2Dist;
                                        if(i==0) minDist = linkDist;
                                        //determine if this node is the closest one so far
                                        if (linkDist<=minDist){
                                                minDist = linkDist;
                                                linkToCutIndex = i;
                                        }
                                ...
                                //cutLink(m_links[linkToCutIndex].m_n[0], m_links[linkToCutIndex].m_n[0]);
				//can also push the indices of the link's nodes... might be easier for rendering
                                //linksCut.push_back(linkToCutIndex);
...
// I think this cutting method works. The refine function takes care of all of the links, nodes, and faces destroyed
//    by the cutting. The parameters for the implicit sphere and the refine function were taken from the bullet
//    example SoftDemo.cpp. The linksCut array is one idea I had for keeping track of the links that are cut. This 
//    array would be accessed in the model plugin and sent to the visual plugin which would not render links in this
//    list. In this case the visual plugin would have to do the work of figuring out which vertices and faces 
//    correspond to each link that has been cut. The other option would be to create a new kind of message that can 
//    send Links and faces along with node positions and the visual plugin could just blindly recreate everything
//    in the message. 
// Another issue is that right now this allows any object to cut the softbody. To fix this I think we would have to put
//    a tag in btRigidBody that can be checked in the btSoftBodyInternals::DoNode(btSoftBody::Node& n) (where this 
//    function is called from).
void btSoftBody::cutOnContact(btSoftBody::Node& n){
  //if(cut){
  //ImplicitSphere isphere(n.m_x,1); //the refining volume
  //refine(&isphere,.0001,true);
    //linksCut.push_back(linkToCutIndex);
...
//	const btRigidBody *tmpRigid = btRigidBody::upcast(colObjWrap->getCollisionObject());
	...
	//std::cout<<"before m_sparsesdf\n";
	//std::cout<<"->Testing\n---wtr.invXform(x): "<<wtr.invXform(x)<<"\n---shp: "<<shp<<"\n---nrm: "<<nrm<<"\n---margin: "<<margin<<std::endl;
	//std::cout<<"---m_worldInfo: "<<m_worldInfo<<std::endl;
	//m_worldInfo->m_sparsesdf;
	//std::cout<<"---m_worldInfo->m_sparsesdf: "<<std::endl;
	//std::cout << "worldInfo evaluate " << worldInfo << std::endl;
	btScalar dst = 
...
{
  //std::cout<<"btSoftBody::updateBounds() check m_worldInfo address: "<<m_worldInfo<<std::endl;
	...
	/*if( m_acceleratedSoftBody )
	{
		...
		if(m_ndbvt.m_root)
		{
			...
			{		
			  //std::cout<<"btSoftBody::updateBounds: before setAabb()\n-->check dispatcher: "<<m_worldInfo->m_dispatcher<<"\n-->check broadphase: "<<m_worldInfo->m_broadphase<<std::endl;
...
{
    //added by hudson 
  // btSoftBody::cutOnContact();
  //std::cout<<"btSoftBody::defaultCollisionHandler()\n";
	...
	case	fCollision::SDF_RS:
		{
		  //std::cout<<"case 1\n";
			...
			//std::cout<<"--end btSoftBody::defaultCollisionHandler() \n";
