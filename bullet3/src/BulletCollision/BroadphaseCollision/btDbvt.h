...
inline void		btDbvt::collideTV(	const btDbvtNode* root,
								  ...
								  DBVT_IPOLICY) const
{
  ///std::cout<<"btDbvt::collideTV()\n";
	DBVT_CHECKTYPE
		if(root)
		{
		  //std::cout<<"--in if (root exists)\n";
			...
			//std::cout<<"--before do-while: stack size is "<<stack.size()<<std::endl;
		    

			do	{
				const btDbvtNode*	n=stack[stack.size()-1];
				stack.pop_back();
				//std::cout<<"-- --in do (before if)\n";
				...
				if(Intersect(n->volume,volume))
				{
					...
					{
					  //std::cout<<"-- --is not internal (policy.process())\n";
					  policy.Process(n);
					}
				}
			} while(stack.size()>0);
		}
	//std::cout<<"--out of collideTv()\n";
