		...
		if (collisionShape->isConcave())
		{
			...
			if (collisionShape->isCompound())
			{
				...
				if (tree) {
					...
					//std::cout<<"call of collideTV from btCollisionWorld\n";
...
struct btSingleContactCallback : public btBroadphaseAabbCallback
{
	...
	virtual bool	process(const btBroadphaseProxy* proxy)
	{
	  //std::cout << "dispatch: process" << std::endl;
		btCollisionObject*	collisionObject = (btCollisionObject*)proxy->m_clientObject;
		...
		if(m_resultCallback.needsCollision(collisionObject->getBroadphaseHandle())) 
		{
			...
			if (algorithm)
			{
				...
				//discrete collsion detection query
				//std::cout << "dispatch: process" << std::endl;

				algorithm->processCollision(&ob0,&ob1, m_world->getDispatchInfo(),&contactPointResult);
				//std::cout << "dispatch: process exit" << std::endl;

				algorithm->~btCollisionAlgorithm();
				m_world->getDispatcher()->freeCollisionAlgorithm(algorithm);
			}
		}
		//std::cout << "dispatch: process out" << std::endl;
		return true;
	}
};
...
{
  //std::cout << "btCollisionWorld::contactPairTest" << std::endl;
