...
//****new from hudson
  //this->softLink = nullptr;
}
////////////////////////////////////////////////// //****new from hudson
void BulletLink::setSoftLink(btSoftBody* bsb)
{ 
  //this->softLink = bsb;
}
//////////////////////////////////////////////////
...
void BulletLink::Init()
{
  ...
  // Create a construction info object for the rigid body
  ...
  //Create the Worldinfo object for the softbody link
  //btSoftBodyWorldInfo worldInfo = btSoftBodyWorldInfo();

  //Create the new soft Body
  //this->softLink = new btSoftBody(&worldInfo); 
  //this->softLink->setUserPointer(this);
  //this->softLink->setCollisionFlags(this->softLink->getCollisionFlags() | //not sure where these come from (not in btRigidBody) but I hope wherever that is can be accessed by softBodies too
  //    btCollisionObject::CF_CUSTOM_MATERIAL_CALLBACK);
  ...
  btSoftRigidDynamicsWorld *bulletWorld = this->bulletPhysics->GetDynamicsWorld();
  ...
  BulletPhysicsPtr bulletPhysicsCopy = this->bulletPhysics;

  //std::cout<<bulletPhysicsCopy->GetType()<<std::endl;
  //std::cout<<"have added the ridid Body\n";
...
void BulletLink::Fini()
{
  if (this->bulletPhysics && this->rigidLink)
  {
    ...
    //bulletWorld->removeCollisionObject(this->softLink);

    delete this->rigidLink;
    //delete this->softLink;
  }
...
void BulletLink::SetGravityMode(bool _mode)
{
  ...
  else
  {
    ...
    //this->softLink->addForce(btVector3(g.X(), g.Y(), g.Z()));
...
void BulletLink::SetLinearVel(const math::Vector3 &_vel)
{
  ...
  //this->softLink->setVelocity(BulletTypes::ConvertVector3(_vel));
  }
...
//
void BulletLink::addSoftLinkToWorld(btSoftBody* psb)
{
  // std::cout<<"just started adding soft link to world.\n";
  //unsigned int categortyBits = GZ_ALL_COLLIDE;
  //unsigned int collideBits = GZ_ALL_COLLIDE;
  ...
  //std::cout<<"added\n";
  //bulletWorld->addSoftBody(this->softLink);
  bulletWorld->addSoftBody(psb);

}
void BulletLink::initSoftBodyPhysics(btSoftBody* psb){
      psb->m_worldInfo->m_dispatcher = this->bulletPhysics->getDispatcher();
      psb->m_worldInfo->m_broadphase = this->bulletPhysics->getBroadPhase();
      psb->m_worldInfo->m_sparsesdf.Initialize();
      // psb->m_worldInfo->m_sparsesdf.Reset();
      psb->m_worldInfo->air_density     	=	(btScalar)1.2;
      psb->m_worldInfo->water_density	=	0;
      psb->m_worldInfo->water_offset    	=	0;
      psb->m_worldInfo->water_normal   	=	btVector3(0,0,0);
      psb->m_worldInfo->m_gravity.setValue(0,-10,0);

      //std::cout<<"BulletLink::initSoftBodyPhysics-- m_worldInfo: "<<psb->m_worldInfo<<std::endl;
